...
# --- TABLA DE FAVORITOS (Relación Usuario <-> Recurso) ---
tabla_favoritos = db.Table('favoritos',
    db.Column('id_usuario', db.Integer, db.ForeignKey('usuarios.id_usuario'), primary_key=True),
    db.Column('id_recurso', db.Integer, db.ForeignKey('recursos.id_recurso'), primary_key=True)
)
...
class Usuario(db.Model):
    ...
    rol = db.Column(db.String(15), default='bibliotecario') # 'admin' o 'alumno'
    grupo = db.Column(db.String(20), nullable=True) # NUEVO: Para alumnos
    ...
    mis_favoritos = db.relationship('Recurso', secondary=tabla_favoritos, lazy='subquery',
...
def inicio():
    ...
    return render_template('index.html', pdfs=pdfs, audios=audios, fisicos=fisicos, bios=bios, efemerides=efemerides, videos=videos, busqueda_activa=busqueda)
...
def admin_login():
    if 'loggedin' in session: 
        if session.get('user_rol') == 'alumno': return redirect(url_for('alumno_dashboard'))
    ...
    if request.method == 'POST':
        ...
        if user and check_password_hash(user.password_hash, request.form['password']):
            ...
            if user.rol == 'alumno': return redirect(url_for('alumno_dashboard'))
            return redirect(url_for('admin_dashboard'))
...
def admin_dashboard():
    if 'loggedin' not in session or session.get('user_rol') == 'alumno': return redirect(url_for('admin_login'))
    today = date.today()
...
def nuevo_recurso():
    ...
    if request.method == 'POST':
        try:
            ...
            nuevo = Recurso(titulo=titulo, autor=autor, tipo_recurso=tipo, descripcion=descripcion, categoria=categoria, ejemplares_total=ejemplares_total, ejemplares_disponibles=ejemplares_total, ruta_archivo_e2=ruta_final, ruta_miniatura=ruta_miniatura_final)
...
# --- RUTAS DE ALUMNOS (FASE 1) ---

@app.route('/alumno/dashboard')
def alumno_dashboard():
    if 'loggedin' not in session or session.get('user_rol') != 'alumno':
        return redirect(url_for('admin_login'))
    
    usuario = db.session.get(Usuario, session['user_id'])
    # Mostrar libros aleatorios o novedades
    libros_sugeridos = Recurso.query.filter(Recurso.tipo_recurso.in_(['pdf', 'audio', 'video'])).limit(6).all()
    
    return render_template('alumno/dashboard.html', 
                           usuario=usuario, 
                           sugeridos=libros_sugeridos)

@app.route('/admin/registrar-alumno', methods=['GET', 'POST'])
def registrar_alumno():
    ...
    if request.method == 'POST':
        ...
        password_simple = request.form.get('password')
        
        primer_nombre = nombre.split()[0].lower()
        usuario_auto = f"{primer_nombre}.{grupo.lower().replace(' ', '')}"
        
        if Usuario.query.filter_by(email=usuario_auto).first():
            flash(f'El usuario {usuario_auto} ya existe. Intenta agregar el apellido.', 'warning')
        else:
            hashed = generate_password_hash(password_simple, method='pbkdf2:sha256')
            ...
            db.session.commit()
            flash(f'¡Alumno registrado! Su usuario es: {usuario_auto}', 'success')
            
    alumnos = Usuario.query.filter_by(rol='alumno').order_by(Usuario.grupo).all()
    return render_template('admin/registrar_alumno.html', alumnos=alumnos, user_name=session.get('user_name'))

@app.route('/alumno/toggle-favorito/<int:recurso_id>')
def toggle_favorito(recurso_id):
    if 'loggedin' not in session: return redirect(url_for('inicio'))
    
    usuario = db.session.get(Usuario, session['user_id'])
    libro = db.session.get(Recurso, recurso_id)
    
    if libro:
        if libro in usuario.mis_favoritos:
            usuario.mis_favoritos.remove(libro)
        else:
            usuario.mis_favoritos.append(libro)
        db.session.commit()
        
    return redirect(request.referrer or url_for('alumno_dashboard'))

# --- RUTAS DE VISUALIZACIÓN SEGURA ---
